...
exports.calculateCarbonProfile = calculateCarbonProfile;
function calculateCarbonProfile(input) {
    const { transport, electricity, water, food, shopping, waste } = input;
    ...
    if (food.dietType === "vegan")
        foodEmissions = 2.0;
    else if (food.dietType === "vegetarian")
        foodEmissions = 3.5;
    else if (food.dietType === "balanced")
        foodEmissions = 5.5;
    else if (food.dietType === "meat-heavy")
        foodEmissions = 8.5;
    ...
    const sustainabilityScore = Math.max(10, Math.min(100, Math.round(100 - (dailyCarbon * 2.2))));
    return {
        dailyCarbon,
        monthlyCarbon,
        annualCarbon,
