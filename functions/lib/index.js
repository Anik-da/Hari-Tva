...
exports.leaderboard = exports.climateRisk = exports.translator = exports.scanVision = exports.generateMission = exports.mentor = exports.calculateCarbon = exports.session = void 0;
...
async function verifyRequestUser(req) {
    ...
    try {
        ...
        const role = decodedToken.role || "citizen";
        return {
...
// ==========================================
exports.calculateCarbon = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
    ...
    try {
        const CalculatorSchema = zod_1.z.object({
            transport: zod_1.z.object({
                carType: zod_1.z.enum(["ice", "ev", "none"]),
                carMiles: zod_1.z.number().nonnegative(),
                transitMiles: zod_1.z.number().nonnegative(),
                flightHours: zod_1.z.number().nonnegative(),
            }),
            electricity: zod_1.z.object({
                kwh: zod_1.z.number().nonnegative(),
                renewablePct: zod_1.z.number().min(0).max(100),
            }),
            water: zod_1.z.object({
                gallons: zod_1.z.number().nonnegative(),
            }),
            food: zod_1.z.object({
                dietType: zod_1.z.enum(["meat-heavy", "balanced", "vegetarian", "vegan"]),
            }),
            shopping: zod_1.z.object({
                spendAmount: zod_1.z.number().nonnegative(),
            }),
            waste: zod_1.z.object({
                bags: zod_1.z.number().nonnegative(),
                recycled: zod_1.z.boolean(),
                composted: zod_1.z.boolean(),
            }),
        });
...
// ==========================================
exports.mentor = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
...
Answer concisely. Return valid JSON only matching:
{
  ...
  "suggestions": ["Task 1", "Task 2"]
        ...
        const rawResponse = await (0, gemini_1.askGeminiPro)(parsed.data.message, systemInstruction);
        let parsedJson;
        try {
...
// ==========================================
exports.generateMission = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
...
Return valid JSON matching:
{
  ...
  "difficulty": "easy"  // MUST be "easy", "medium", or "hard"
        ...
        const rawResult = await (0, gemini_1.askGeminiPro)(prompt, "You are a green coach. Return JSON only.");
        let missionData;
        try {
...
// ==========================================
exports.scanVision = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
    ...
    try {
        const VisionScanSchema = zod_1.z.object({
            ...
            type: zod_1.z.enum(["product", "waste", "food", "receipt", "electricity_bill"]),
        });
        ...
        const aiResponseText = await (0, gemini_1.askGeminiVision)(image, mimeType, prompt);
        let parsedResult;
        try {
...
// ==========================================
exports.translator = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
...
// ==========================================
exports.climateRisk = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
        ...
        const prompt = `Analyze climate risks and resource stress indicators for: ${location}. Format response in valid JSON matching:
{
  ...
  "summary": "Brief summary explanation."
        ...
        const rawResponse = await (0, gemini_1.askGeminiPro)(prompt, "You are a climate risk model. Return JSON only.");
        let result;
        try {
...
// ==========================================
exports.leaderboard = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== "GET") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
